...
if (isset($_POST['tname']) && !empty($_POST['cpf']) && !empty($_POST['date_nasc']) && !empty($_POST['email'])) {
    ...
    if (!empty($tname) && !empty($cpf) && !empty($date_nasc) && !empty($email) && !empty($id)) {
        $user->updateDocente($tname, $cpf, $date_nasc, $email, $id);
        if ($user = true) {
            echo "<p class='alert alert-success'><b>Docente</b> atualizado com sucesso!</p>";
            echo "<a href='getAllDocente.php'><button type='button' class='style btn btn-danger'>Voltar</button></a>";
            exit;
...
<h1 class="title">Alterar Docente</h1>
        ...
        <div class="col-sm-5">
            <form method="post" class="style_form">
                <div class="form-group">
                    ...
                    <input type="text" name="tname" value="<?= $info['tname'];?>" class="form-control" id="exampleInputName" placeholder="Nome do docente">
                </div>
                <div class="form-group">
                    <label for="exampleInputCpf">CPF: </label>
                    <input type="text" name="cpf" value="<?= $info['cpf'];?>" class="form-control" id="exampleInputCpf" placeholder="CPF do docente">
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    ...
                    <input type="email" name="email" value="<?= $info['email'];?>" class="form-control" id="exampleInputEmail1" placeholder="E-mail do docente">
                </div>
                <br>
